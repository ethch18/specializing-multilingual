...
def load_conllu(file, skip_checks=False):
    ...
    class UDWord:
        def __init__(self, span, columns, is_multiword):
            ...
            self.is_functional_deprel = (
                self.columns[DEPREL] in FUNCTIONAL_DEPRELS
            )
    ...
    while True:
        ...
        # Basic tokens/words
            ...
            try:
                ...
                raise UDError(
                    "Cannot parse word ID '{}'".format(_encode(columns[ID]))
                )
            ...
            try:
                ...
                raise UDError(
                    "Cannot parse HEAD '{}'".format(_encode(columns[HEAD]))
                )
            if head_id < 0:
...
def evaluate(gold_ud, system_ud):
    ...
    def alignment_score(
        alignment, key_fn=None, filter_fn=None, bypass_fn=None
    ):
        ...
        for words in alignment.matched_words:
            if filter_fn is None or filter_fn(words.gold_word):
                if (
                    bypass_fn is not None and bypass_fn(words.gold_word)
                ) or key_fn(words.gold_word, gold_aligned_gold) == key_fn(
    ...
    def find_multiword_span(gold_words, system_words, gi, si):
        ...
        while not beyond_end(
            gold_words, gi, multiword_span_end
        ) or not beyond_end(system_words, si, multiword_span_end):
        ...
        for g in reversed(range(gi - gs)):
            for s in reversed(range(si - ss)):
                if (
                    ...
                    lcs[g][s] = 1 + (
                        lcs[g + 1][s + 1]
                        if g + 1 < gi - gs and s + 1 < si - ss
                        else 0
                    )
                lcs[g][s] = max(
                    lcs[g][s], lcs[g + 1][s] if g + 1 < gi - gs else 0
                )
                lcs[g][s] = max(
                    lcs[g][s], lcs[g][s + 1] if s + 1 < si - ss else 0
                )
        return lcs
    ...
    if gold_ud.characters != system_ud.characters:
        ...
        _gold = "".join(map(_encode, gold_ud.characters[index : index + 20]))
        _system = "".join(map(_encode, system_ud.characters[index : index + 20]))
        print("Difference in gold, system")
        print(f"\tGold: {_gold}")
        ...
        #         "".join(
        #             map(_encode, system_ud.characters[index : index + 20])
        #         ),
        #     )
        # )
    ...
    return {
        ...
        "Lemmas": alignment_score(
            alignment,
            lambda w, ga: w.columns[LEMMA]
            if ga(w).columns[LEMMA] != "_"
            else "_",
        ),
            ...
            if args.counts:
                ...
                print(
                    "{:11}|{:10.2f} |{:10.2f} |{:10.2f} |{}".format(
                        ...
                        "{:10.2f}".format(
                            100 * evaluation[metric].aligned_accuracy
                        )
